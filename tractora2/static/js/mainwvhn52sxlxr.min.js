!function (global, factory) {
    ...
    wrapMap.optgroup = wrapMap.option, wrapMap.tbody = wrapMap.tfoot = wrapMap.colgroup = wrapMap.caption = wrapMap.thead, wrapMap.th = wrapMap.td, jQuery.extend({
        clone: function (elem, dataAndEvents, deepDataAndEvents) {
            ...
            for (fragment.textContent = "", i = 0; elem = nodes[i++];)if ((!selection || jQuery.inArray(elem, selection) === -1) && (contains = jQuery.contains(elem.ownerDocument, elem), tmp = getAll(fragment.appendChild(elem), "script"), contains && setGlobalEval(tmp), scripts))for (j = 0; elem = tmp[j++];)rscriptType.test(elem.type || "") && scripts.push(elem);
            return fragment
        ...
        var t, o = "mCustomScrollbar", a = "mCS", n = ".mCustomScrollbar", i = {
            ...
            init: function (t) {
                ...
                return e(o).each(function () {
                    var o = e(this);
                    o.data(a) && (o.data(a), X.call(this, "remove"), k.call(this), t && B.call(this), M.call(this, !0), o.addClass(d[3]))
                })
            }, destroy: function () {
                var t = f.call(this);
                return e(t).each(function () {
    ...
    t = "undefined" != typeof exports && null !== exports ? exports : this, t.Lethargy = function () {
        ...
        return t.prototype.check = function (t) {
            ...
            var s, i, l, a, e, n, h;
            return s = -1 === t ? this.lastDownDeltas : this.lastUpDeltas, null === s[0] ? t : !(this.deltasTimestamp[2 * this.stability - 2] + this.delay > Date.now() && s[0] === s[2 * this.stability - 1]) && (l = s.slice(0, this.stability), i = s.slice(this.stability, 2 * this.stability), h = l.reduce(function (t, s) {
                return t + s
            }), e = i.reduce(function (t, s) {
                return t + s
            }), n = h / l.length, a = e / i.length, Math.abs(n) < Math.abs(a * this.tolerance) && this.sensitivity < Math.abs(a) && t)
    ...
    m = {
        ...
        if (this.settings.loop) {
            var e = c - this.relative(this.current()), f = this.current(), g = this.current(), h = this.current() + e, i = 0 > g - h, j = this._clones.length + this._items.length;
        ...
        b = b === d ? this._items.length : this.normalize(b, !0), this.trigger("add", {
            content: a,
            position: b
        }), 0 === this._items.length || b === this._items.length ? (this.$stage.append(a), this._items.push(a), this._mergers.push(1 * a.find("[data-merge]").andSelf("[data-merge]").attr("data-merge") || 1)) : (this._items[b].before(a),
            this._items.splice(b, 0, a), this._mergers.splice(b, 0, 1 * a.find("[data-merge]").andSelf("[data-merge]").attr("data-merge") || 1)), this.invalidate("items"), this.trigger("added", {
            content: a,
            position: b
        })
    ...
    var lethargy, MOUSE_EVENTS_STRING = "mousewheel DOMMouseScroll wheel MozMousePixelScroll";
    "undefined" != typeof Lethargy && null !== Lethargy && (lethargy = new Lethargy);
    ...
    $.smartscroll = function (overrides) {
        ...
        if (options.sectionSelector || (options.sectionSelector = "." + options.sectionClass), ("undefined" == typeof EventEmitter || null === EventEmitter || options.eventEmitter && options.eventEmitter.constructor !== EventEmitter) && (options.eventEmitter = null), options.bindSwipe)var xDown = null, yDown = null, handleTouchStart = function (e) {
            var e = e.originalEvent || e;
            xDown = e.touches[0].clientX, yDown = e.touches[0].clientY
        }, handleTouchMove = function (e) {
            var e = e.originalEvent || e;
            if (xDown && yDown) {
                var xUp = e.touches[0].clientX, yUp = e.touches[0].clientY, xDiff = xDown - xUp, yDiff = yDown - yUp;
                Math.abs(xDiff) > Math.abs(yDiff) ? xDiff > 0 ? options.eventEmitter.emitEvent("swipeLeft") : options.eventEmitter.emitEvent("swipeRight") : yDiff > 0 ? options.eventEmitter.emitEvent("swipeUp") : options.eventEmitter.emitEvent("swipeDown"), xDown = null, yDown = null
            }
        };
        var sectionWrapperTop, sectionWrapperBottom, isScrolling = !1, sections = [], validBreakPoint = !1, belowBreakpoint = !1, currentHash = window.location.hash, sectionWrapper = $(options.sectionWrapperSelector + ":first"), sectionWrapperIsVisible = function () {
            var windowTop = getWindowTop(), windowBottom = windowTop + $(window).height();
            return windowBottom > sectionWrapperTop && windowTop <= sectionWrapperBottom
        }, scrollToPixel = function (pixel, speed) {
            isScrolling || (isScrolling = !0, $("body,html").stop(!0, !0).animate({scrollTop: pixel}, speed, function () {
                isScrolling = !1, options.eventEmitter && options.eventEmitter.emitEvent("scrollEnd")
            }))
        };
        this.scroll = function (down) {
            if (sections) {
                ...
                if (options.eventEmitter) {
                    var sectionIndexAtWindowMiddle = getSectionIndexAt(windowTop + $(window).height() / 2), nextSlideNumber = down ? sectionIndexAtWindowMiddle + 1 : sectionIndexAtWindowMiddle - 1;
                    options.eventEmitter.emitEvent("scrollStart", [nextSlideNumber])
                }
                for (var i = 0; i < sections.length; i++)if (windowTop < sections[i])return down ? scrollToPixel(sections[i], 700) : scrollToPixel(sections[i - 1] - $(window).height(), 700), options.eventEmitter && options.eventEmitter.emitEvent("scrollEnd"), !1
            }
        };
        var calculateSectionBottoms = function () {
            var tmpSections = [];
            sectionWrapperTop = Math.round(sectionWrapper.position().top + parseInt(sectionWrapper.css("paddingTop"), 10) + parseInt(sectionWrapper.css("borderTopWidth"), 10) + parseInt(sectionWrapper.css("marginTop"), 10)), sectionWrapperBottom = Math.round(sectionWrapperTop + sectionWrapper.height(), 10), tmpSections.push(sectionWrapperTop), $(options.sectionSelector).each(function (i, el) {
                tmpSections.push(Math.round(sectionWrapperTop + $(el).position().top + $(el).outerHeight()))
            }), sections = tmpSections
        }, getScrollAction = function (e) {
            if (lethargy)var validScroll = lethargy.check(e);
            if (!isScrolling)if (lethargy) {
                if (1 === validScroll)return "up";
                if (validScroll === -1)return "down"
            } else {
            ...
            for (var i = 0; i < sections.length; i++)if (position <= sections[i])return i;
            return sections.length
        }, bindScroll = function () {
            $(window).bind(MOUSE_EVENTS_STRING, function (e) {
                var scrollAction = getScrollAction(e);
                options.dynamicHeight && calculateSectionBottoms(), sectionWrapperIsVisible();
                var windowTop = getWindowTop(), windowBottom = windowTop + $(window).height();
                if (windowBottom > sectionWrapperTop && windowTop <= sectionWrapperBottom) {
                    var sectionIndexAtWindowTop = getSectionIndexAt(windowTop), sectionIndexAtWindowMiddle = getSectionIndexAt(windowTop + $(window).height() / 2), sectionIndexAtWindowBottom = getSectionIndexAt(windowBottom);
                    sectionIndexAtWindowTop === sectionIndexAtWindowBottom && options.innerSectionScroll || (e.preventDefault(), e.stopPropagation(), scrollAction && ("up" === scrollAction ? (options.toptotop ? scrollToPixel(sections[sectionIndexAtWindowMiddle - 2] + 1, options.animationSpeed) : scrollToPixel(sections[sectionIndexAtWindowMiddle - 1] - $(window).height(), options.animationSpeed), options.eventEmitter && options.eventEmitter.emitEvent("scrollStart", [sectionIndexAtWindowMiddle - 1])) : "down" === scrollAction && (scrollToPixel(sections[sectionIndexAtWindowMiddle] + 1, options.animationSpeed), options.eventEmitter && options.eventEmitter.emitEvent("scrollStart", [sectionIndexAtWindowMiddle + 1]))))
                }
            })
        }, unbindScroll = function () {
            $(window).unbind(MOUSE_EVENTS_STRING)
        };
        if (sectionWrapper.css({position: "relative"}), setTimeout(function () {
                if (calculateSectionBottoms(), options.autoHash && (null === options.eventEmitter || options.hashContinuousUpdate || options.eventEmitter.addListener("scrollEnd", autoHash)), options.initialScroll && currentHash.length > 0) {
                    var matchedObject = $('[data-hash="' + currentHash.substr(1) + '"]');
                    matchedObject.length > 0 && scrollToPixel(matchedObject[0].offsetTop + sectionWrapperTop, 0)
                }
            }, 50), $(window).bind("resize", calculateSectionBottoms), null !== options.breakpoint && options.breakpoint === parseInt(options.breakpoint, 10) && options.breakpoint > 0 && (validBreakPoint = !0), "vp" == options.mode)if (options.ie8) {
            var resizeToVP = function () {
                $(options.sectionSelector).css({height: $(window).height()})
            };
            resizeToVP(), $(window).bind("resize", resizeToVP)
        } else $(options.sectionSelector).css({height: "100vh"});
        if (options.sectionScroll && (validBreakPoint && $(window).bind("resize", function (e) {
                if ($(window).width() < options.breakpoint) {
                    if (!belowBreakpoint)return unbindScroll(), belowBreakpoint = !0, !1
                } else belowBreakpoint && (bindScroll(), belowBreakpoint = !1)
            }), bindScroll()), options.bindSwipe && ($(window).on("touchstart", handleTouchStart), $(window).on("touchmove", handleTouchMove)), options.bindKeyboard) {
            var handleKeydown = function (e) {
                e = e.originalEvent || e, options.dynamicHeight && calculateSectionBottoms();
                var windowTop = getWindowTop(), windowBottom = windowTop + $(window).height();
                if (sectionWrapperIsVisible()) {
                    var sectionIndexAtWindowTop = getSectionIndexAt(windowTop), sectionIndexAtWindowMiddle = getSectionIndexAt(windowTop + $(window).height() / 2), sectionIndexAtWindowBottom = getSectionIndexAt(windowBottom);
                    if (sectionIndexAtWindowTop !== sectionIndexAtWindowBottom || !options.innerSectionScroll)switch (e.which) {
                        case 38:
                            e.preventDefault(), e.stopPropagation(), options.toptotop ? scrollToPixel(sections[sectionIndexAtWindowMiddle - 2] + 1, options.animationSpeed) : scrollToPixel(sections[sectionIndexAtWindowMiddle - 1] - $(window).height(), options.animationSpeed), options.eventEmitter && options.eventEmitter.emitEvent("scrollStart", [sectionIndexAtWindowMiddle - 1]);
                            break;
                        case 40:
                            e.preventDefault(), e.stopPropagation(), scrollToPixel(sections[sectionIndexAtWindowMiddle] + 1, options.animationSpeed), options.eventEmitter && options.eventEmitter.emitEvent("scrollStart", [sectionIndexAtWindowMiddle + 1]);
                            break;
                        default:
                            return
                    }
